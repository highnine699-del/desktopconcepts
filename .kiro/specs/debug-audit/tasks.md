...
- [x] **Task 2 — REQ-C2: Fix RefreshScheduler asset name lookup**
  ...
  - Change `"Setup.exe"` exact match to `Contains("DesktopConcepts-Setup") && EndsWith(".exe")`.
...
- [x] **Task 7 — REQ-M1: Implement buffer deduplication**
  - File 1: `src/DesktopConcepts.Domain/IConceptBufferStore.cs` — add `PeekConceptsAsync`.
  - File 2: `src/DesktopConcepts.Infrastructure/Storage/JsonConceptBufferStore.cs` — implement it.
  - File 3: `src/DesktopConcepts.Application/Schedulers/CloudPrefetchService.cs` — replace
...
- [x] **Task 9 — REQ-M3: Fix HttpClient Timeout registration**
  - File 1: `src/DesktopConcepts.UI/App.xaml.cs` — add `AddHttpClient("UpdateDownload", c => c.Timeout = TimeSpan.FromMinutes(10))`.
  - File 2: `src/DesktopConcepts.Application/Schedulers/RefreshScheduler.cs` — change
...
- [x] **Task 10 — REQ-L1: Add ForceRetryAsync, wire to ErrorRetry_Click**
  - File 1: `src/DesktopConcepts.Application/Schedulers/ConceptGenerationBackgroundService.cs`
    — add public `ForceRetryAsync(CancellationToken)`.
  - File 2: `src/DesktopConcepts.UI/Views/WidgetWindow.xaml.cs` — add
...
- [x] **Task 12 — REQ-L3: Replace silent install with user-consent banner**
  - File 1: `src/DesktopConcepts.Application/Schedulers/RefreshScheduler.cs`
  ...
  - File 2: `src/DesktopConcepts.UI/Views/WidgetWindow.xaml` — add `UpdateBanner` panel.
  - File 3: `src/DesktopConcepts.UI/Views/WidgetWindow.xaml.cs` — add
    `RefreshScheduler _refreshScheduler` constructor parameter; add `OnUpdateAvailable`,
    `UpdateNow_Click`, `UpdateLater_Click` handlers.
  - File 4: `src/DesktopConcepts.UI/App.xaml.cs` — wire `UpdateAvailable` event.

- [x] **Task 13 — REQ-A1: Remove service locator from WidgetWindow**
  - File 1: `src/DesktopConcepts.UI/App.xaml.cs` — register `Func<SettingsWindow>` factory.
  - File 2: `src/DesktopConcepts.UI/Views/WidgetWindow.xaml.cs`
