    ...
    private async Task CheckForUpdateAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (IsNewerVersion(latest, currentVersion))
            {
                ...
                // Find the installer asset — name contains "Quire-Setup" and ends ".exe"
                ...
                {
                    _logger.LogWarning(
                        "Update v{Latest} available but no Quire-Setup*.exe asset found in release.",
                        latest);
                }
            }
            else
    ...
    public async Task PerformUpdateAsync(string downloadUrl, CancellationToken cancellationToken)
    {
        try
        {
            var tempPath = Path.Combine(
                Path.GetTempPath(), $"Quire-Update-{Guid.NewGuid()}.exe");
