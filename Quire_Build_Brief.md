...
DesktopConcepts.sln
├── src/
│   ├── Quire.Domain/          (no NuGet deps beyond BCL)
│   ├── Quire.Application/     (depends on Domain only)
│   ├── Quire.Infrastructure/  (depends on Domain)
│   └── Quire.UI/              (WPF, depends on Application + Infrastructure for DI wiring only)
└── tests/
    └── Quire.Tests/
```

`Quire.Domain.csproj` — target `net8.0`, no WPF reference. This project must never reference
`System.Windows` or `System.Net.Http`. If it needs to, something is wired wrong.

`Quire.UI.csproj` — `<UseWPF>true</UseWPF>`, `<OutputType>WinExe</OutputType>`.
...
- [ ] Uninstall → nothing left outside `%AppData%\Quire`
